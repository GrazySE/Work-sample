...
if (isset($_POST['username'], $_POST['email'], $_POST['p'])) {
   ...
   // check existing email  
    ...
    if (empty($error_msg)) {
        ...
        if ($insert_stmt = $mysqli->prepare("INSERT INTO login (username, email, password) VALUES (?, ?, ?)")) {
            $insert_stmt->bind_param('sss', $username, $email, $password);
            // Execute the prepared query.
            if (! $insert_stmt->execute()) {
                header('Location:https://ifcpraktik.hannahbanana.se/ifc/error.php?err=Registration failure: INSERT');
            }
        }
        header('Location:https://ifcpraktik.hannahbanana.se/ifc/register_success.php');
    }
}
?>
